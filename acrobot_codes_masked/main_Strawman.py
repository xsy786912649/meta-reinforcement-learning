...
for i in range(TRAIN_TASK_COUNT-1, TRAIN_TASK_COUNT):
    ...
    torch.save(strawman.policy, f"results/Strawman/run_{RUN}/models/model_{i}")
    torch.save(strawman.value_function, f"results/Strawman/run_{RUN}/models/value_function_{i}")
    torch.save(strawman.cost_value_function_1, f"results/Strawman/run_{RUN}/models/cost_value_function_1_{i}")
    torch.save(strawman.cost_value_function_2, f"results/Strawman/run_{RUN}/models/cost_value_function_2_{i}")
